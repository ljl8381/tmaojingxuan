...
+ (Renren *)newRenRen{
    ...
    newRenrenObject.appKey = kRENRENWEIBOAPIKEY;
    newRenrenObject.appId = kRENRENWEIBOAPPID;
...
+ (Renren *)sharedRenren {
    if (!sharedRenren) {
        ...
        sharedRenren.appKey = kRENRENWEIBOAPIKEY;
        sharedRenren.appId = kRENRENWEIBOAPPID;
    }
    return sharedRenren;
}
...
- (void)requestWithParam:(RORequestParam *)param andDelegate:(id <RORequestDelegate>)delegate {
    if (nil == param.method || [param.method length] <= 0) {
        TRACE(@"API Method must be specified");
        return;
    }
    
    if (![self isSessionValid]) {
        TRACE(@"Session is not valid! Request abort!!");
        return;
    }
...
- (void)request:(RORequest *)request didFailWithError:(NSError*)error{
    TRACE(@"Failed to expire the session");
}
...
-(void)getFriendsInfo:(ROGetFriendsInfoRequestParam *)param andDelegate:(id<RenrenDelegate>)delegate
{
	...
	if (![self isSessionValid]) {
		//[self delUserSessionInfo];
		TRACE(@"Session not valid!! Request abort!");
		return;
	}
